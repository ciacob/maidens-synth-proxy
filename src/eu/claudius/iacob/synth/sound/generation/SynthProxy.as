package eu.claudius.iacob.synth.sound.generation {
    ...
    public class SynthProxy extends EventDispatcher implements ISynthProxy {
        ...
        private static var _deliveredSounds:Object = [];

        private var _synth:ISynthEngine;
         ...
         * @constructor
         */
        public function SynthProxy(audioStorage:ByteArray, synth:ISynthEngine = null) {
            _annotationTasks = [];
            _noteStorages = {};
        ...
        /**
         * Retrieves the raw content of a `.sf2` (SoundFont) file corresponding to the given `preset` number.
         *
         ...
         *
         * @param   preset
         *          The preset number used to locate the corresponding `.sf2` file in the application's directory.
         *
         * @return  A `ByteArray` containing the `.sf2` file’s contents, or `null` if:
         *          - The file has already been delivered previously.
         *          - No matching `.sf2` file (or default fallback) is found.
         */
        private function _getSoundsForPreset(preset:int):ByteArray {
            if (!_soundsCache[preset]) {
                preset = FileAssets.DEFAULT_SOUNDS_FILE_KEY;
            }
            if (!_deliveredSounds[preset]) {
                _deliveredSounds[preset] = true;
        ...
        private static function _renderSamples(milliseconds:Number, samplesStorage:ByteArray,
                $samplesBuffer:ByteArray = null, synth:ISynthEngine = null,
            ...
            if (synth != null) {
                ...
                while (samplesBuffer.length < dueBytes) {
                    synth.getSamples(samplesBuffer);
                }
            }
        ...
        private static function _renderTailSamples(samplesStorage:ByteArray, synth:Object):void {
            ...
            do {
                ...
                synth.getSamples(samplesStorage);
